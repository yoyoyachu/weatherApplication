...
const weatherBlock = async (weather_periods) => {

    weather_periods.forEach(async (element,index) => {
        setTimeout(async function(){
            ...
            let markup = `
                <div class='card-outerdiv' id=${element.number}">
...
async function locationForecast(search_string){
    ...
    const weather_periods =await weather_data['properties']['periods'];
    return [city, state, weather_periods]
}
...
async function Wrapper(stringForSearch) {
    let {city, state, weather_periods} =await locationForecast('clifton');
    cityName(city, state);

}

document.getElementById('main_searchBtn').addEventListener('click', async(e) => {
    e.preventDefault();
    document.getElementById('main_page').style.display = 'none';
    let stringForSearch = document.getElementById('main_search').value;
    
    let [city, state, weather_periods] = await locationForecast(stringForSearch);
    cityName(city, state);
    await weatherBlock(weather_periods);
    document.getElementById('nav_container').style.display = 'inline';
});


document.getElementById('form_second').addEventListener('submit', async(e) => {
    e.preventDefault();
    let stringForSearch = document.getElementById('search_field').value;
    let [city, state, weather_periods] = await locationForecast(stringForSearch);
    cityName(city, state);
    refreshPage();
    console.log(cityName(city, state))
    await weatherBlock(weather_periods);
});
